export function Story() {
  return (
    <section className="grid items-center gap-16 border-t border-rule-cream px-12 py-20 lg:grid-cols-[460px_1fr]">
      <div className="relative aspect-[4/5] overflow-hidden rounded-[28px] bg-cream-2">
        <div
          className="absolute inset-0"
          style={{
            background:
              "radial-gradient(circle at 70% 30%, rgba(232,201,122,0.5), transparent 55%), repeating-linear-gradient(135deg, transparent 0 24px, rgba(33,33,33,0.05) 24px 25px)",
          }}
        />
      ...
      <div>
        <h2 className="m-0 mb-6 text-balance text-[64px] font-bold leading-none tracking-[-0.035em]">
          ...
          <em className="not-italic text-clay">starting</em> was the hardest
          part.
        </h2>
        <p className="m-0 mb-4 max-w-[540px] text-[17px] leading-relaxed text-charcoal-soft">
        ...
        <div className="mt-6 text-[15px] font-semibold">
          — The Nudo team
          <small className="mt-0.5 block font-normal text-mute">
            Brewed and packed somewhere with bad parking
          </small>
        </div>
        <a
          href="#"
          className="mt-5 inline-flex items-center gap-2 border-b-2 border-warm pb-1 text-[15px] font-semibold text-charcoal no-underline"
        >
          Read our story →
        </a>
      </div>
